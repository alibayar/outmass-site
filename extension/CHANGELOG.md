...
- **Lists written in your own alphabet now load.** Excel saves in whatever character set your computer uses, and until now OutMass could only read UTF-8 and the two Chinese ones — so a contact list written in Russian, Ukrainian, Arabic, Hebrew, Greek or Thai was rejected outright. OutMass now also reads the alphabet your browser says you use, including when your browser menus are in English but your data is not. Lists in Turkish, Polish, Baltic or Western European character sets are not covered yet: those encodings are indistinguishable from one another by inspection, and reading one wrongly would put a mangled name into an email you send, so for now OutMass still asks you to re-save those as "CSV UTF-8" rather than guess.
