...
| 12 | 🟠 med | No in-Outlook affordance to discover the sidebar; documented shortcut doesn't exist | `extension/content_script.js`, `extension/styles/content.css` | ✅ (FAB launcher) |
...
> **All 13 fixed** on branch `fix/user-loss-leaks` (3 fix commits + this tracker). Backend (7) deploys via Railway on merge; extension (6) ships in the **v0.1.17** store package. Reviewed each diff against source; syntax/`node --check` + adjacent backend tests pass.
