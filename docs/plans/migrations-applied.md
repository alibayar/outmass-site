...
|---|---|---|
| 036_campaign_stalled_notice | 2026-09-04, Ali (in chat) | ✅ 2026-09-05 — `information_schema.columns` for `campaigns.stalled_notice_at` returned one row: `timestamp with time zone`, nullable YES. Verified BEFORE the customer email that depends on it: the guard is inert without this column, so faisal@samaed.com's "does not resume on its own any more" would have been false while the code looked correct. Backend `7ead074` was already live and answering. Existence is the whole requirement here — the guard turns itself on by asking whether the row from `select *` carries the key — so `information_schema` answers the question that matters, unlike 034 where a `.get()` path could read a missing column as empty. WRITES are not yet proven and cannot be without holding a real campaign; a rejected write is safe by design (the campaign is held, nothing sends, and it logs ERROR and counts `held_stale_unnotified`). |
